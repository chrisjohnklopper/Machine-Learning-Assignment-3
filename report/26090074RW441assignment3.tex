...
\begin{table}[htbp]
	...
	\resizebox{\columnwidth}{!}{
		\csvautobooktabular{../csv-descriptions/summary-statistics-for-apple.csv}
	}
...
As we can see the hidden layer $h_t$ in a Jordan RNN is updated using a state layer vector $s_t$ (context units) instead of the previous hidden layer $h_{t-1}$. The state layer vector is updated using the previous state $s_{t-1}$ and the previous output $y_{t-1}$.
...
An Elman RNN was trained using the Apple stock data. The loss function used was mean-squared error. The \texttt{torch.optim.Adam} Adam optimizer was used with \texttt{learning\_rate}=0.002, which was increased from the original amount of 0.001 after it was noticed that the training was taking long to find the local minima.
The test versus validation performance was carefully monitored using plots throughout the folds. 
...
The size of the hidden layer is 32. Only 1 layer was used, as instructed. A $tanh$ activation function was used. Bias parameters were used. No drop-out was applied. Batch size was 16. The model was trained for 25 epochs for each fold. A sequence length of 64 was used. At fold 7, the test and validation performance matched, indicating best generalisation performance. The performance degraded significantly as the number of epochs increased. 8-fold time series cross-validation was used.

\begin{figure}[htbp]
	\centering
	\includegraphics[width=1.0\linewidth]{../images/elman_hidden32_test_vs_val_fold7.pdf}
	\caption{Elman RNN Test vs. Validation Performance (hidden layer size 32, fold 7)}
	\label{fig:example}
\end{figure}

In fold 8, the MSE was as low as 13.0346 for the validation data, although it must be noted that the scale of the data is important here since the price of the stock increases by very little every day (for most days), so a low MSE isn't as meaningful. We can see however some local optimum is reached and the performance is not bad.

\section{Empirical process}

\subsection{Best simple RNN was chosen}
